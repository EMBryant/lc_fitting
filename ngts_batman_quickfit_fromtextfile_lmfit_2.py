...
def lc_fit(fn, period, epoc, objid, fit_t0=False):
    ...
    phase = np.array(time, dtype=float)
    
   # phase = ((time - epoc)/period)%1  #Convert time values in to phases
   # phase = np.array([p-1 if p > 0.8 else p for p in phase], dtype=float)
    
    p_fit = phase[phase < 0.1]  #Crop phase and flux arrays to only contain values
    f_fit = flux[phase < 0.1]   #in range (-0.2 ,  0.2)
    e_fit = err[phase < 0.1]
    ...
    p_best = np.linspace(-0.1, 0.1, 10000)     #Produce a model LC using 
...
if __name__ == '__main__':
    ...
    if args.fit_t0:
        p_best, f_best, p_fit, f_fit, e_fit, rp_best, a_best, inc_best, t0_best, t_dur, t_depth, res = lc_fit(args.fn, args.per, args.epoc, args.objid, fit_t0=True)
    else:
        p_best, f_best, p_fit, f_fit, e_fit, rp_best, a_best, inc_best, t_dur, t_depth, res = lc_fit(args.fn, args.per, args.epoc, args.objid, fit_t0=False)
