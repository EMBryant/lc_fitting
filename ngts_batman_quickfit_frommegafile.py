...
def ParseArgs():
    ...
    parser.add_argument('field', type=str, help="NGTS field name")
    parser.add_argument('cycle', type=str, help="NGTS Cycle")
    parser.add_argument('objid', type=int, help="Object IDs")
...
def lc_fit(args):
    ...
    This version pulls the time, flux, error, period, and transit epoch out of the NGTS Megafiles
    '''
    #Load time and flux data for the object
    dic = ngtsio.get(args.field, args.cycle, ['HJD', 'SYSREM_FLUX3', 'FLUX3_ERR', 'PERIOD', 'EPOCH', 'FLUX_MEAN'], obj_id=args.objid)
    
    time = dic['HJD'] / 86400.                     #time in units of days
    flux = dic['SYSREM_FLUX3'] / dic['FLUX_MEAN']  #Flux set st oot flux is 1.
    err = dic['FLUX3_ERR'] / dic['FLUX_MEAN']      #Flux errors  
    period = dic['PERIOD'] / 86400.                #Orbital period in units of days
    epoc = dic['EPOCH'] / 86400.                   #Time of first transit centre
...
if __name__ == '__main__':
    ...
    time1 = time.time()
    p_best, f_best, p_fit, f_fit, rp_best, a_best, inc_best, t_dur, t_depth = lc_fit(args)
    time2 = time.time()
    print("Time taken: {:.4f} s".format(time2-time1))
