...
def lc_min(params, time, flux, err):
	...
	poly_model = params['c2'].value*phase**2 + params['c1'].value*phase + params['c0'].value
	model_overall = transit_model * poly_model
	residuals = (flux - model_overall)**2/err**2
	return residuals

def lc_min_spl(params, time, flux, err, hw):
	'''
	Function which calculates Chi2 value for a given set of input parameters.
	Function to be minimized to find best fit parameters
	...
	bw = 30 / (1440*params['per'].value)                    #Bin width - 10 mins in units of phase
	p_bin, f_bin, e_bin = lc_bin(phase, flux, err, bw)

	pbin_oot = p_bin[np.abs(p_bin) > hw]
	fbin_oot = f_bin[np.abs(p_bin) > hw]
	ebin_oot = e_bin[np.abs(p_bin) > hw]

	spl = IUS(pbin_oot, fbin_oot, w=ebin_oot, k=3)
	...
	residuals = flux - model_total/err
	return residuals
...
if __name__ == '__main__':
    ...
    poly = res.params['c2'].value*p_best**2 + res.params['c1'].value*p_best + res.params['c0'].value
