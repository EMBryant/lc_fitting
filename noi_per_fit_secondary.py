...
if __name__ == '__main__':
 ...
 #   t0_best = 7500.199548181591
 #   per_best = 0.5779297215002647

    params=Parameters()         #Parameter instance to hold fit parameters
    params.add('rp', value=0.05, min=0., max=0.5)    #Planet:Star radius ratio
    params.add('a', value=10., min=0., max=30.)    #Semi-major axis
  #  params.add('inc', value=89.5, min=89., max=90.)  #Orbital inclination
    ...
    params.add('base', value=1, min=0, max=2)
    params.add('fp', value=0.06668, min=0, max=0.1)
    
    res = minimize(lc_min, params, args=(time, flux, err), method='leastsq', calc_covar=True) #perform minimization
    ...
    pm_best.t_secondary = 0                 #Time of transit centre
    ...
    pm_best.fp = res.params['fp'].value
	
    p_best = np.linspace(-0.5, 0.5, 20000)     #Produce a model LC using 
    m_best = bm.TransitModel(pm_best, p_best, transittype="secondary")  #the output best fit parameters
    f_best = m_best.light_curve(pm_best)-base
    ...
    bw = 0.5 / (1440*per_best)                    #Bin width - 10 mins in units of phase
    ...
    plt.plot(p_best, f_best, 'g-', linewidth=2, zorder=2)
